...
class Circuit():
    ...
    def __init__(self, num_ref, input_state):
        ...
        if num_ref > 3 or num_ref < 0:
            raise Exception('Compression valid for only 2 and 3 qubits, for now!')
        else:
    ...
    def swap_test(system_state):
        ...
        """
        c_swap = fredkin(7, control=0, targets=[1, 3])*fredkin(7, control=0, targets=[2, 4])
    ...
    def measure_overlap(meas_state, state):
        """ Measure Overlap
        """
        hadamard = tensor(snot(), qeye(2), qeye(2), qeye(2), qeye(2), qeye(2), qeye(2))
    ...
    def rotation_xy(theta, phi):
        """Rotation XY
        """
        return Q.expm(-1j * theta * (np.cos(phi)*sigmax() + np.sin(phi)*sigmay())/2)
    ...
    def get_unitary(self, circuit, params):
        """Get Unitary
        """
        return self.unitary_a(params) if circuit == 'a' else self.unitary_b(params)
    ...
    def unitary_a(self, params):
        ...
        unitary_4 = swap(4, targets=[0, 1]) * unitary_2 * swap(4, targets=[1, 0])
        unitary_5 = swap(4, targets=[1, 2]) * unitary_3 * swap(4, targets=[2, 1])

        unitary_6 = swap(4, targets=[2, 3]) * swap(4, targets=[1, 2]) * unitary_1 \
            * swap(4, targets=[3, 2]) * swap(4, targets=[2, 1])

        output_unitary = unitary_1 *unitary_2*unitary_3*unitary_4*unitary_5*unitary_6
    ...
    def unitary_b(self, params):
        ...
        #
        output_unitary = rotate_all * c_rotate_qb0 * c_rotate_qb1 * c_rotate_qb2 * c_rotate_qb3 * rotate_all
    ...
    def single_qubit_gate(self, theta, phi, phi_z):
        """Single Qubit Gate
        """
        return self.rotation_xy(theta, phi) * self.rotation_z(phi_z)

    def V_gate(self, alpha, beta, delta):
        """V
        """

        op1 = tensor(self.rotation_xy(beta, np.pi/2), self.rotation_xy(3*np.pi/2, delta))
        op2 = tensor(self.rotation_xy(alpha, 0), self.rotation_xy(3*np.pi/2, 0))
